...
interface Contract { id: string; contract_no: string; customer_name: string; }
...
const expenseTypes = ['rent', 'salaries', 'courtFees', 'lawyerFees', 'utilities', 'office', 'transport', 'other'];
...
export default function ExpensesPage() {
  ...
  async function loadData() {
    setLoading(true);
    const [expRes, custRes, contRes] = await Promise.all([
      supabase.from('expenses').select('*').order('created_at', { ascending: false }),
      supabase.from('customers').select('id, customer_no, name'),
      supabase.from('contracts').select('id, contract_no, customer_name'),
    ]);
  ...
  const exportHeaders = [t('expenseVoucherNo'), t('expenseDate'), t('expenseType'), t('amount'), t('description'), t('customerName'), t('contractNo')];
  const exportRows = filtered.map(e => [e.expense_voucher_no, e.expense_date, e.expense_type, e.amount, e.description, e.customer_name, e.contract_no]);
                  ...
                  {filtered.map(e => (
                    <tr key={e.id} className="border-b border-slate-100 hover:bg-blue-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <Badge className={typeColor(e.expense_type)} variant="secondary">{t(e.expense_type as keyof typeof t)}</Badge>
                      </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.expense_type} onChange={e => setForm({ ...form, expense_type: e.target.value })}>
                  {expenseTypes.map(et => <option key={et} value={et}>{t(et as keyof typeof t)}</option>)}
                </select>
              </div>
              <div>
              ...
              {(form.expense_type === 'courtFees' || form.expense_type === 'lawyerFees') && (
                <div>
