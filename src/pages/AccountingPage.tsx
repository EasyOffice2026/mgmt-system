...
type ActiveView = 'overview' | 'report' | 'income';
...
export default function AccountingPage() {
  ...
  const reportTypes: { key: ReportType; label: string; icon: typeof BarChart3; color: string; gradient: string }[] = [
    ...
    { key: 'finished', label: t('finishedCases'), icon: Calculator, color: 'text-emerald-600', gradient: 'from-emerald-500 to-emerald-600' },
  ...
  async function loadReport(type: ReportType) {
    ...
    if (type === 'sales') {
      ...
      const res = await supabase.from('contracts').select('*').gte('start_date', dateFrom).lte('start_date', dateTo);
      rows = (res.data || []).filter((c: any) => c.status === 'finished').map((c: any) => ({
        id: c.id, date: c.start_date || '', description: `${c.contract_no || ''} - ${c.customer_name || ''}`,
        amount: c.sale_price || 0, customer: c.customer_name || '', status: c.status || '',
      }));
    } else if (type === 'legal') {
  ...
  async function loadIncomeStatement() {
    ...
    const finishedCases = contracts.filter((c: any) => c.status === 'finished').length;
    ...
    const finishedValue = contracts.filter((c: any) => c.status === 'finished').reduce((s: number, c: any) => s + (c.sale_price || 0), 0);
                ...
                <CardContent className="p-0">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      ...
                      <tr className="border-b border-slate-100 hover:bg-green-50/50">
                        <td className="py-3 px-6"><Badge className="bg-green-100 text-green-700" variant="secondary">{t('finishedCases')}</Badge></td>
